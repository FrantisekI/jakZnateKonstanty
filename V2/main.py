...
def konstanty(jakSeJmenuje='Anonym', Placeholder_Pi=0, piNaCoZna='3,', Placeholder_E=0, eNaCoZna='2,', Placeholder_Fi=0, fiNaCoZna='1,'):
    global GpiNaCoZna, GPlaceholder_Pi, GeNaCoZna, GPlaceholder_E, GfiNaCoZna, GPlaceholder_Fi
    ...
    return render_template("konstanty.html", napis='Vítej, mohl bych tě poprosit o vyplnění následujícího "dotazníku"?',
                           placeholderJmeno=jakSeJmenuje, tvojeIP=request.remote_addr, 
...
def submit():
    ...
    data = {
        ...
        'jmeno': Gjmeno,
        'znalost_PI': GPlaceholder_Pi,
        'znalost_E': GPlaceholder_E,
        'znalost_FI': GPlaceholder_Fi,
        'IP': get_ip()
    }
    date = datetime.datetime.now().strftime("%Y%m%d %H%M%S%f")
    #create new json file 
    with open(f'V2/userOutput/{date}.json', 'w') as outfile:
        json.dump(data, outfile)


    return render_template("konstanty.html", napis='Děkuji za vyplnění dotazníku!',
                           placeholderJmeno=Gjmeno, tvojeIP=request.remote_addr, 
